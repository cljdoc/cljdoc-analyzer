...
A library to return publics for namespaces in Clojure/ClojureScript source code projects with the understanding that
this data is sometimes altered in those projects at load time.
...
=== general use
TODO: design phase only - not implemented

You can use cljdoc-analyzer ad hoc to get data for a project published to a known maven repo.

If the project you want to analyze is up on a maven repo:
[source,bash,no-wrap]
----
clojure -m cljdoc.analyzer.main --project io.aviso/pretty --version "0.1.29"
----
...
* project specific overrides for languages (aka platforms), namespaces and deps will be moved to a local config file.
...
The integration tests can take a while to run, to focus on unit tests only:
----
clojure -A:test --skip-meta integration
----

To focus on integration tests only:
----
clojure -A:test --focus-meta integration
----

To run in watch mode tack on a `--watch` to any of the above commands. See kaocha docs for other options.
