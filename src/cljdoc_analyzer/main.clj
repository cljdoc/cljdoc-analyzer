...
(defn analyze [{:keys [project version output-filename]}]
  (let [{:keys [jar pom]} (deps/resolve-dep (symbol project) version)]
    (runner/analyze! {:project project
                      :version version
                      :jarpath jar
                      :pompath pom
  ...
  {:app {:command "cljdoc-analyzer"
   ...
   [{:command     "analyze"
     ...
     :opts        [{:option "project" :short "p" :as "Project to import" :type :string :default :present}
                   {:option "version" :short "v" :as "Project version to import" :type :string :default :present}
                   {:option "output-filename" :short "o" :as "Where to write edn output" :type :string :default :present}]
     :runs        analyze}]})
